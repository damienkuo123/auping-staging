...
def main():
    ...
    try:
        for r in c["routes"]:
            ...
            html=re.sub(r'data-auping-page-id="[^"]*"',f'data-auping-page-id="cs-wave3-{r["slug"]}"',html,count=1)
            html=html.replace("<main",f'<!-- AUPING_CS_WAVE3_FAQ_V1 route="{r["route"]}" --><main',1)
            target.parent.mkdir(parents=True,exist_ok=True)
            target.write_text(html)
            created.append(target)
        print("MATERIALIZE_FAQ_CLUSTER_PASS",len(created))
    except Exception:
        for p in created:
            try: p.unlink()
