...
def main():
    ...
    try:
        for r in c["routes"]:
            ...
            html=meta_replace(html,"property","og:title",r["localizedTitle"])
            html=meta_replace(html,"property","og:description",r["localizedDescription"])
            html=meta_replace(html,"name","twitter:title",r["localizedTitle"])
            html=meta_replace(html,"name","twitter:description",r["localizedDescription"])
            ...
            html=re.sub(r'data-auping-page-id="[^"]*"',f'data-auping-page-id="cs-wave4-{r["slug"]}"',html,count=1)
            if 'data-auping-wave4-hero="mobile"' in frag:
                css='''<style data-auping-wave4-responsive-v11>
[data-auping-wave4-hero="mobile"]{display:none!important}
@media (max-width:767px){
[data-auping-wave4-hero="desktop"]{display:none!important}
[data-auping-wave4-hero="mobile"]{display:block!important}
}
</style>'''
                html=html.replace("</head>",css+"</head>",1)
            html=html.replace("<main",f'<!-- AUPING_CS_WAVE4_SPECIAL_V1_1 route="{r["route"]}" --><main',1)
            target.parent.mkdir(parents=True,exist_ok=True)
            target.write_text(html)
            created.append(target)
        print("MATERIALIZE_SPECIAL_SUPPORT_PASS",len(created))
    except Exception:
        for p in created:
            try:p.unlink()
